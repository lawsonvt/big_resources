...
out_dir <- paste0(root_dir, "results/subclustering_opc/")
...
subset_seu <- subset(seu_obj, subset = cell_type == "OPC")
...
clusters <- sort(unique(subset_seu$opc_clusters))
...
saveRDS(subset_seu@meta.data, paste0(out_dir, "subset_opc.metadata.RDS"))
