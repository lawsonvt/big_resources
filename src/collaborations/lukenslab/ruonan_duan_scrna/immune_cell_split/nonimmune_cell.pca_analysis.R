...
out_dir <- paste0(root_dir, "results/immune_cell_split/nonimmune_cell.pca_analysis/")
...
levels(metadata$nonimmune_clusters) <- paste0("cluster", levels(metadata$nonimmune_clusters))
...
cluster_pseudo <- AggregateExpression(
  ...
  assays = "RNA",
  slot = "counts",
  group.by = c("nonimmune_clusters","sample")
)
...
clusters <- levels(metadata$nonimmune_clusters)

# do just the first 21 clusters, since later ones have missing cell counts
clusters <- clusters[1:21]
...
plot_grid(plotlist = pca_plot_list, ncol=7)
ggsave(paste0(out_dir, "all_clusters_pca_plot.png"), width=18, height=10)
