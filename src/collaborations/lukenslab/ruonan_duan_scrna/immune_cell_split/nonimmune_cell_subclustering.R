...
out_dir <- paste0(root_dir, "results/immune_cell_split/nonimmune_cell_subclustering/")
...
subset_clusters <- "7,9,10,15,17,19,22,25,27,29,30,34,35,36"
...
ElbowPlot(subset_seu, ndims=50) + 
  labs(title="Non-Immune Subset") +
  scale_x_continuous(breaks=seq(0,50,5)) +
  scale_y_continuous(breaks=seq(0,50,5), limits=c(0,NA))
ggsave(paste0(out_dir, "nonimmune_subset.pre_doublet.pca_elbow_plot.png"), width=6, height=5, bg="white")
...
subset_seu <- FindClusters(subset_seu, cluster.name = "nonimmune_clusters")
...
subset_sce <- scDblFinder(subset_sce, clusters="nonimmune_clusters", BPPARAM=SerialParam(RNGseed = 1234),
                          samples = "sample")
...
ggsave(paste0(out_dir, "nonimmune.doublet_qc_plots.png"), width=9, height=6)
...
ggsave(paste0(out_dir, "nonimmune.doublet_counts.png"), width=4, height=5)

dbl_meta <- as.data.frame(colData(subset_sce))

saveRDS(dbl_meta, file=paste0(out_dir, "nonimmune.doublet_output.RDS"))
...
ElbowPlot(subset_seu, ndims=50) + 
  ...
  scale_x_continuous(breaks=seq(0,50,5)) +
  scale_y_continuous(breaks=seq(0,50,5), limits=c(0,NA))
ggsave(paste0(out_dir, "nonimmune_subset.post_doublet.pca_elbow_plot.png"), width=6, height=5, bg="white")
...
# create umap
subset_seu <- RunUMAP(subset_seu, dims = 1:max_pc_dim, reduction="pca", reduction.name="umap.nonimmune_pca")

DimPlot(subset_seu, reduction="umap.nonimmune_pca", group.by= "nonimmune_clusters",
        label=T) 
ggsave(paste0(out_dir, "nonimmune_subcluster.umap.png"), width=7, height=5)

DimPlot(subset_seu, reduction="umap.nonimmune_pca", group.by= "nonimmune_clusters",
        label=T, split.by = "sample", ncol = 3) 
ggsave(paste0(out_dir, "nonimmune_subcluster.per_sample.umap.png"), width=14, height=7)
...
ggplot(subset_meta,
  ...
  facet_wrap(~ nonimmune_clusters, ncol=6,
             scales="free_y") +
  theme_bw() +
  theme(axis.text.x = element_text(angle=90, hjust=1))
ggsave(paste0(out_dir, "nonimmune_cluster_counts.sample_bar_plot.png"), width=10, height=8)
...
ggplot(subset_meta,
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle=90, hjust=1))
ggsave(paste0(out_dir, "nonimmune_cluster_counts.condition_bar_plot.png"), width=10, height=8)

clusters <- sort(unique(subset_seu$nonimmune_clusters))
...
Idents(subset_seu) <- "nonimmune_clusters"
...
saveRDS(subset_seu@meta.data, paste0(out_dir, "subset_nonimmune.metadata.RDS"))

SaveSeuratRds(subset_seu, paste0(out_dir, "subset_nonimmune.seurat.RDS"))
