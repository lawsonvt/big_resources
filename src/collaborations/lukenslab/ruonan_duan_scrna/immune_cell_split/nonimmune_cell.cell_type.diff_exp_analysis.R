...
out_dir <- paste0(root_dir, "results/immune_cell_split/nonimmune_cell.cell_type.diff_exp_analysis/")
...
# add in celltype
metadata$nonimmune_cell_type <- "Other"

# identify OPCs
metadata[metadata$nonimmune_clusters %in% c("2","18","19","20"),]$nonimmune_cell_type <- "OPC"

seu_obj$nonimmune_cell_type <- metadata$nonimmune_cell_type

# look at cell type breakdown
celltype_clusters <- as.data.frame(table(metadata$cell_type,
                                         metadata$nonimmune_clusters))
...
seu_pseudo <- AggregateExpression(
  ...
  assays = "RNA",
  slot = "counts",
  group.by = c("nonimmune_cell_type","sample")
)
