...
subset_seu <- subset(seu_obj, subset = cell_type == "T Cell")
...
clusters <- sort(unique(subset_seu$tcell_clusters))
...
Idents(subset_seu) <- "tcell_clusters"
...
SaveSeuratRds(subset_seu, paste0(out_dir, "subset_tcell.seurat.RDS"))
