...
subset_seu <- subset(seu_obj, subset = cell_type == "Microglia")
...
DimPlot(subset_seu, reduction="umap.harmony_filtered", group.by= "microglia_clusters",
        label=T) 
ggsave(paste0(out_dir, "microglia_subcluster.original_umap.png"), width=7, height=5)

DimPlot(subset_seu, reduction="umap.microglia_pca", group.by= "microglia_clusters",
        label=T, split.by = "condition") 
ggsave(paste0(out_dir, "microglia_subcluster.per_condition.umap.png"), width=10, height=5)

DimPlot(subset_seu, reduction="umap.microglia_pca", group.by= "microglia_clusters",
        label=T, split.by = "sample", ncol = 3) 
ggsave(paste0(out_dir, "microglia_subcluster.per_sample.umap.png"), width=14, height=7)
...
# cluster counts
micro_meta <- subset_seu@meta.data
...
Idents(subset_seu) <- "microglia_clusters"
