...
subset_sce <- scDblFinder(subset_sce, clusters="astrocyte_clusters", BPPARAM=bp)
...
ggsave(paste0(out_dir, "astrocyte.doublet_qc_plots.png"), width=9, height=6)
...
ggsave(paste0(out_dir, "astrocyte.doublet_counts.png"), width=4, height=5)

dbl_meta <- as.data.frame(colData(subset_sce))

saveRDS(dbl_meta, file=paste0(out_dir, "astrocyte.doublet_output.RDS"))
...
ElbowPlot(subset_seu, ndims=50) + 
  ...
  scale_x_continuous(breaks=seq(0,50,5)) +
  scale_y_continuous(breaks=seq(0,50,5), limits=c(0,NA))
ggsave(paste0(out_dir, "astrocyte_subset.post_doublet.pca_elbow_plot.png"), width=6, height=5, bg="white")
...
DimPlot(subset_seu, reduction="umap.astrocyte_pca", group.by= "astrocyte_clusters",
        label=T) 
ggsave(paste0(out_dir, "astrocyte_subcluster.umap.png"), width=6, height=5)

DimPlot(subset_seu, reduction="umap.astrocyte_pca", group.by= "astrocyte_clusters",
        label=T, split.by = "condition") 
ggsave(paste0(out_dir, "astrocyte_subcluster.per_condition.umap.png"), width=9, height=5)
