...
out_dir <- paste0(root_dir, "results/gsea_analysis.sex_time_comp/")
...
results_file <- paste0(root_dir, "results/differential_expression_analysis.sex_time_comp/total_results.RDS")

results <- readRDS(results_file)

for (age in names(results)) {
  ...
  for (cell in names(age_results)) {
    ...
    for (contrast in names(cell_results)) {
      ...
      output_stump <- paste0(out_dir, age, ".", cell, ".", contrast, ".gsea_results")
