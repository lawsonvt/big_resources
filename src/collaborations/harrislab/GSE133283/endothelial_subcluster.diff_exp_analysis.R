...
out_dir <- paste0(root_dir, "results/endothelial_subcluster.diff_exp_analysis/")
...
seu_obj$endothelial_clusters <- metadata$endothelial_clusters
...
# just do all of them
clusters <- levels(seu_obj$endothelial_clusters)
...
seu_pseudo <- AggregateExpression(
  ...
  assays = "RNA",
  slot = "counts",
  group.by = c("endothelial_clusters","sample_id")
)
...
ggsave(paste0(out_dir, "cluster_deg.volcanoes.png"), width=12, height=10, bg="white")
