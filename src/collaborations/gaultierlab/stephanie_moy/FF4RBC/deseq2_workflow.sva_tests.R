...
out_dir <- paste0(root_dir, "FF4RBC/results/deseq2_workflow_sva_tests/")
...
saveRDS(list(dds=dds_total,
             res=res_total), file=paste0(out_dir, "total_sva_results.RDS"))
...
saveRDS(list(dds=dds_free,
             res=res_free), file=paste0(out_dir, "free_sva_results.RDS"))
...
saveRDS(list(dds=dds_1sv,
             res=res_1sv), file=paste0(out_dir, "1sv_sva_results.RDS"))

# Overlap analysis --------

deg_list <- list(
  "Total SVA" = res_total[res_total$padj < 0.05,]$gene_id,
  "Free SVA" = res_free[res_free$padj < 0.05,]$gene_id,
  "1SV SVA" = res_1sv[res_1sv$padj < 0.05,]$gene_id
)
...
ggVennDiagram(deg_direct_list)

sum(deg_direct_list$`Total SVA` %in% deg_direct_list$`Free SVA` &
