...
out_dir <- paste0(root_dir, "FF4RBC/results/gsea_workflow.no_sva/")
...
results_file <- paste0(root_dir, "FF4RBC/results/deseq2_workflow.no_sva/deg_results.RDS")
results_list <- readRDS(results_file)

results_list <- list("treatment-control"=results_list)
...
gsea_results <- lapply(results_list, function(degs) {
  ...
  # clean up DEGs
  degs <- degs[!is.na(degs$gene_name),]
...
for (contrast in names(gsea_results)) {
  ...
  contrast_results <- gsea_results[[contrast]]$gsea_results
  
  write.xlsx(contrast_results, 
             file=paste0(results_dir, contrast, ".gsea_results.xlsx"),
             colWidths="auto")
